...
#ifndef FILE_PVM_OLD_NSEI_H
#define FILE_PVM_OLD_NSEI_H
...
#define DDSOLD_ISINTEL (*((unsigned char *)(&DDS_INTEL) + 1) == 0)
#define DDSOLD_ERRORMSG() DDSV3Old::errormsg(__FILE__,__LINE__)
#define DDSOLD_MEMERROR() DDSV3Old::errormsg(__FILE__,__LINE__)
#define DDSOLD_IOERROR() DDSV3Old::errormsg(__FILE__,__LINE__)
...
public:
  ...
  inline unsigned int DDS_readbits (unsigned int bits)
  {
    ...
    {
      DDS_bufsize -= bits;
      value = DDSV3Old::DDS_shiftr(DDS_buffer, DDS_bufsize);
    }
    else
    {
      value = DDSV3Old::DDS_shiftl(DDS_buffer, bits - DDS_bufsize);

      if (DDS_cachepos >= DDS_cachesize) DDS_buffer = 0;
      else
      {
        DDS_buffer = *((unsigned int *)&DDS_cache[DDS_cachepos]);
        if (DDSOLD_ISINTEL) DDSV3Old::DDS_swapuint(&DDS_buffer);
        DDS_cachepos += 4;
      }
    ...
    DDS_buffer &= DDSV3Old::DDS_shiftl(1, DDS_bufsize) - 1;
  ...
  inline void DDS_loadbits (unsigned char* data, unsigned int size)
  {
    ...
    if ((DDS_cache = (unsigned char*)realloc(DDS_cache, DDS_cachesize + 4)) == NULL) DDSOLD_MEMERROR();
    *((unsigned int *)&DDS_cache[DDS_cachesize]) = 0;

    DDS_cachesize = 4 * ((DDS_cachesize + 3) / 4);
    if ((DDS_cache = (unsigned char*)realloc(DDS_cache, DDS_cachesize)) == NULL) DDSOLD_ERRORMSG();
  }
